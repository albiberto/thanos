...
public static class ZobristHasher
{
    ...
    public static long CalculateHash(in WarArena arena)
    {
        ...
        if (!me.Dead)
        {
            ...
            foreach (var part in meBodyPart1) hash ^= ZobristTable.GetSnakeValue(me.Id, part);

            // Iteriamo sulla seconda parte del corpo (se esiste)
            foreach (var part in meBodyPart2) hash ^= ZobristTable.GetSnakeValue(me.Id, part);
        }
        else
        {
        ...
        for (var i = 0; i < arena.Enemies.Count; i++)
        {
            ...
            if (!enemy.Dead)
            {
                ...
                foreach (var part in enemyBodyPart1) hash ^= ZobristTable.GetSnakeValue(enemy.Id, part);

                foreach (var part in enemyBodyPart2) hash ^= ZobristTable.GetSnakeValue(enemy.Id, part);
            }
            else
            {
    ...
    public static long CalculateHash(in Request request, Dictionary<string, int> snakeIdMap)
    {
        ...
        {
            var snakeId = snakeIdMap[snake.Id];
