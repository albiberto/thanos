...
public class TestRequest(TestGame game, int turn, TestBoard board, TestSnakeHealth you)
{
    [JsonPropertyName("game")] public TestGame Game { get; } = game;

    [JsonPropertyName("turn")] public int Turn { get; } = turn;

    [JsonPropertyName("board")] public TestBoard Board { get; } = board;

    [JsonPropertyName("you")] public TestSnakeHealth You { get; } = you;
}
...
public class TestGame(Guid id, TestRuleset ruleset, string map, string source, int timeout)
{
    ...
    [JsonPropertyName("ruleset")] public TestRuleset Ruleset { get; } = ruleset;
...
public class TestRuleset(string name, string version, TestRulesetSettings settings)
{
    ...
    [JsonPropertyName("settings")] public TestRulesetSettings Settings { get; } = settings;
}
...
public class TestRulesetSettings(int foodSpawnChance, int minimumFood, int hazardDamagePerTurn, TestRoyale? royale, TestSquad? squad)
{
    ...
    [JsonPropertyName("royale")] public TestRoyale? Royale { get; } = royale;

    [JsonPropertyName("squad")] public TestSquad? Squad { get; } = squad;
}
...
[method: JsonConstructor]
public class TestBoard(uint height, uint width, TestCoordinate[] food, TestCoordinate[] hazards, TestSnakeHealth[] snakes)
{
    [JsonPropertyName("height")] public uint Height { get; } = height;

    [JsonPropertyName("width")] public uint Width { get; } = width;

    [JsonPropertyName("food")] public TestCoordinate[] Food { get; } = food;

    [JsonPropertyName("hazards")] public TestCoordinate[] Hazards { get; } = hazards;

    [JsonPropertyName("snakes")] public TestSnakeHealth[] Snakes { get; } = snakes;
}

[method: JsonConstructor]
public class TestSnakeHealth(string id, string name, uint health, TestCoordinate[] body, string latency, TestCoordinate head, uint length, string shout, TestCustomizations customizations)
{
    ...
    [JsonPropertyName("body")] public TestCoordinate[] Body { get; } = body;

    [JsonPropertyName("latency")] public string Latency { get; } = latency;

    [JsonPropertyName("head")] public TestCoordinate Head { get; } = head;

    [JsonPropertyName("length")] public uint Length { get; } = length;

    [JsonPropertyName("shout")] public string Shout { get; } = shout;

    [JsonPropertyName("customizations")] public TestCustomizations Customizations { get; } = customizations;
}
